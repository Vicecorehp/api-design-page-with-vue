...
class ChartNode extends Rect {
  ...
  get level(): string {
    return (this.data?.level as string) || DEFAULT_LEVEL
  }

  getLabelStyle(): false | LabelStyleProps {
  ...
  getKeyStyle(attributes: ParsedRectStyleProps) {
    return {
  ...
  getPositionStyle(attributes: ParsedRectStyleProps) {
  ...
  drawPositionShape(attributes: ParsedRectStyleProps, container: DisplayObject) {
    const positionStyle = this.getPositionStyle(attributes)
    this.upsert('position', Label, positionStyle, container)
  }

  getStatusStyle(attributes: ParsedRectStyleProps) {
    if (this.level === 'overview') return false
    return {
  ...
  drawStatusShape(attributes: ParsedRectStyleProps, container: DisplayObject) {
    const statusStyle = this.getStatusStyle(attributes)
    this.upsert('status', Badge, statusStyle, container)
  }

  getPhoneStyle(attributes: ParsedRectStyleProps) {
    if (this.level === 'overview') return false
    return {
  ...
  drawPhoneShape(attributes: ParsedRectStyleProps, container: DisplayObject) {
  ...
  updateZoomLevel = async (e) => {
    if ('scale' in e.data) {
      ...
      const level = Object.entries(this.levels).find(
        ([_key, [min, max]]) => scale > min && scale <= max,
      )?.[0]
